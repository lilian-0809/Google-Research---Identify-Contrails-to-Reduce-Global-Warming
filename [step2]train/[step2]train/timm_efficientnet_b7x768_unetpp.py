...
class Paths:
    ...
    contrails = '/root/Contrails/data/contrails/'
    train_path = '/root/Contrails/data/train_df.csv'
    valid_path = '/root/Contrails/data/valid_df.csv'
    save_dir = '/root/data/save'
...
class UNet(nn.Module):
    def __init__(self, cfg):
        ...
        self.loss_fn = smp.losses.DiceLoss(mode='binary', smooth=2.0)
...
def train(model, train_dataloader, test_dataloader, optimizer, epochs, device):
    ...
    for epoch in range(epochs):
        ...
        if val_dice> best:
            best = val_dice
            PATH = f"{Paths.save_dir}/unetpp_{Config.encoder}_{Config.image_size}_ep{epoch}_dice{val_dice:.4f}.pth"
...
PATH = f"{Paths.save_dir}/{Config.encoder}_th{bi}_dice{bdice:.4f}_final.pt"
torch.save(img, PATH)
